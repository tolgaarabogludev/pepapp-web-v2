import type { Metadata } from 'next'
import Link from 'next/link'
import { getTranslations } from 'next-intl/server'
import { useLocale, useTranslations } from 'next-intl'
import { Header } from '@/components/layout/Header'
import { Footer } from '@/components/layout/Footer'

type PageProps = {
  params: Promise<{ locale: string }>
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'premiumPage' })

  return {
    title: t('meta.title'),
    description: t('meta.description'),
  }
}

const benefitKeys = ['personal', 'insights', 'calm'] as const
const comparisonKeys = ['cycle', 'content', 'premiumContent', 'insights', 'adFree'] as const

const comparisonAvailability = {
  cycle: { free: true, premium: true },
  content: { free: true, premium: true },
  premiumContent: { free: false, premium: true },
  insights: { free: false, premium: true },
  adFree: { free: false, premium: true },
} satisfies Record<(typeof comparisonKeys)[number], { free: boolean; premium: boolean }>

function CheckIcon({ active }: { active: boolean }) {
  return (
    <span
      className={`inline-flex h-6 w-6 items-center justify-center rounded-full text-xs font-semibold ${
        active
          ? 'bg-black text-white dark:bg-white dark:text-black'
          : 'bg-black/5 text-black/30 dark:bg-white/8 dark:text-white/30'
      }`}
      aria-hidden="true"
    >
      {active ? '✓' : '—'}
    </span>
  )
}

export default function PremiumPage() {
  const t = useTranslations('premiumPage')
  const locale = useLocale()

  return (
    <>
      <Header />
      <main className="bg-white text-black dark:bg-[#0b0b0d] dark:text-white">
        <section className="mx-auto max-w-4xl px-6 pb-20 pt-24 sm:px-8 sm:pt-32">
          <p className="text-sm font-medium tracking-[-0.01em] text-black/50 dark:text-white/50">
            {t('hero.eyebrow')}
          </p>
          <h1 className="mt-4 max-w-3xl text-4xl font-semibold tracking-[-0.03em] text-black dark:text-white sm:text-6xl sm:leading-[1.05]">
            {t('hero.title')}
          </h1>
          <p className="mt-6 max-w-2xl text-base leading-7 text-black/65 dark:text-white/65 sm:text-lg sm:leading-8">
            {t('hero.description')}
          </p>
          <div className="mt-10 flex flex-col gap-3 sm:flex-row">
            <Link
              href={`/${locale}#download`}
              className="inline-flex items-center justify-center rounded-full bg-black px-6 py-3 text-sm font-medium text-white transition hover:bg-black/90 dark:bg-white dark:text-black dark:hover:bg-white/90"
            >
              {t('hero.primaryCta')}
            </Link>
            <Link
              href={`/${locale}/about`}
              className="inline-flex items-center justify-center rounded-full border border-black/10 px-6 py-3 text-sm font-medium text-black transition hover:bg-black/[0.03] dark:border-white/15 dark:text-white dark:hover:bg-white/[0.06]"
            >
              {t('hero.secondaryCta')}
            </Link>
          </div>
        </section>

        <section className="mx-auto max-w-5xl px-6 py-20 sm:px-8">
          <div className="max-w-2xl">
            <p className="text-sm font-medium tracking-[-0.01em] text-black/45 dark:text-white/45">
              {t('benefits.eyebrow')}
            </p>
            <h2 className="mt-4 text-3xl font-semibold tracking-[-0.03em] text-black dark:text-white sm:text-4xl">
              {t('benefits.title')}
            </h2>
          </div>

          <div className="mt-12 grid gap-4 md:grid-cols-3">
            {benefitKeys.map((key) => (
              <div
                key={key}
                className="rounded-3xl border border-black/8 bg-black/[0.02] p-6 dark:border-white/10 dark:bg-white/[0.04] sm:p-8"
              >
                <h3 className="text-xl font-medium tracking-[-0.02em] text-black dark:text-white">
                  {t(`benefits.items.${key}.title`)}
                </h3>
                <p className="mt-4 text-sm leading-7 text-black/60 dark:text-white/60 sm:text-base">
                  {t(`benefits.items.${key}.body`)}
                </p>
              </div>
            ))}
          </div>
        </section>

        <section className="mx-auto max-w-4xl px-6 py-20 sm:px-8">
          <div className="rounded-3xl bg-[#faf7fb] px-8 py-12 dark:bg-white/[0.04] sm:px-12 sm:py-16">
            <p className="text-sm font-medium tracking-[-0.01em] text-black/45 dark:text-white/45">
              {t('feel.eyebrow')}
            </p>
            <div className="mt-6 max-w-3xl space-y-4 text-2xl font-medium tracking-[-0.03em] text-black dark:text-white sm:text-4xl sm:leading-tight">
              <p>{t('feel.line1')}</p>
              <p>{t('feel.line2')}</p>
              <p>{t('feel.line3')}</p>
            </div>
          </div>
        </section>

        <section className="mx-auto max-w-5xl px-6 py-20 sm:px-8">
          <div className="grid gap-10 md:grid-cols-[0.9fr_1.1fr] md:items-center">
            <div>
              <p className="text-sm font-medium tracking-[-0.01em] text-black/45 dark:text-white/45">
                {t('intelligence.eyebrow')}
              </p>
              <h2 className="mt-4 text-3xl font-semibold tracking-[-0.03em] text-black dark:text-white sm:text-4xl">
                {t('intelligence.title')}
              </h2>
              <p className="mt-6 text-base leading-8 text-black/65 dark:text-white/65 sm:text-lg">
                {t('intelligence.description')}
              </p>
            </div>

            <div className="rounded-[2rem] border border-black/8 bg-black/[0.02] p-6 dark:border-white/10 dark:bg-white/[0.04] sm:p-8">
              <div className="rounded-3xl bg-white p-6 shadow-sm dark:bg-white/[0.06]">
                <p className="text-sm text-black/45 dark:text-white/45">Pepapp</p>
                <p className="mt-3 text-lg font-medium leading-8 tracking-[-0.02em] text-black dark:text-white">
                  {t('intelligence.pepapp')}
                </p>
              </div>
              <div className="mt-4 rounded-3xl bg-[#ffeff4] p-6 dark:bg-white/[0.08]">
                <p className="text-sm text-black/50 dark:text-white/50">Sen</p>
                <p className="mt-3 text-base leading-7 text-black/70 dark:text-white/70">
                  {t('intelligence.user')}
                </p>
              </div>
            </div>
          </div>
        </section>

        <section className="mx-auto max-w-5xl px-6 py-20 sm:px-8">
          <div className="max-w-2xl">
            <p className="text-sm font-medium tracking-[-0.01em] text-black/45 dark:text-white/45">
              {t('comparison.eyebrow')}
            </p>
            <h2 className="mt-4 text-3xl font-semibold tracking-[-0.03em] text-black dark:text-white sm:text-4xl">
              {t('comparison.title')}
            </h2>
          </div>

          <div className="mt-10 overflow-hidden rounded-3xl border border-black/8 dark:border-white/10">
            <div className="grid grid-cols-[1.4fr_0.8fr_0.8fr] bg-black/[0.02] px-5 py-4 text-sm font-medium text-black/55 dark:bg-white/[0.04] dark:text-white/55 sm:px-8">
              <span>{t('comparison.feature')}</span>
              <span className="text-center">{t('comparison.free')}</span>
              <span className="text-center">{t('comparison.premium')}</span>
            </div>
            {comparisonKeys.map((key) => (
              <div
                key={key}
                className="grid grid-cols-[1.4fr_0.8fr_0.8fr] items-center border-t border-black/8 px-5 py-5 text-sm text-black/70 dark:border-white/10 dark:text-white/70 sm:px-8 sm:text-base"
              >
                <span>{t(`comparison.items.${key}`)}</span>
                <span className="flex justify-center">
                  <CheckIcon active={comparisonAvailability[key].free} />
                </span>
                <span className="flex justify-center">
                  <CheckIcon active={comparisonAvailability[key].premium} />
                </span>
              </div>
            ))}
          </div>
        </section>

        <section className="mx-auto max-w-4xl px-6 pb-28 pt-10 sm:px-8">
          <div className="rounded-3xl border border-black/8 bg-black/[0.02] px-8 py-10 dark:border-white/10 dark:bg-white/[0.04] sm:px-10 sm:py-12">
            <p className="text-sm font-medium tracking-[-0.01em] text-black/45 dark:text-white/45">
              {t('finalCta.eyebrow')}
            </p>
            <h2 className="mt-3 text-2xl font-semibold tracking-[-0.03em] text-black dark:text-white sm:text-3xl">
              {t('finalCta.title')}
            </h2>
            <p className="mt-4 max-w-2xl text-base leading-7 text-black/60 dark:text-white/60">
              {t('finalCta.description')}
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link
                href={`/${locale}#download`}
                className="inline-flex items-center justify-center rounded-full bg-black px-6 py-3 text-sm font-medium text-white transition hover:bg-black/90 dark:bg-white dark:text-black dark:hover:bg-white/90"
              >
                {t('finalCta.primaryCta')}
              </Link>
              <Link
                href={`/${locale}/pepzine`}
                className="inline-flex items-center justify-center rounded-full border border-black/10 px-6 py-3 text-sm font-medium text-black transition hover:bg-black/[0.03] dark:border-white/15 dark:text-white dark:hover:bg-white/[0.06]"
              >
                {t('finalCta.secondaryCta')}
              </Link>
            </div>
          </div>
        </section>
      </main>
      <Footer />
    </>
  )
}